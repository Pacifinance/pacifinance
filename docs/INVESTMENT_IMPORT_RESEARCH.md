...
- **Structure** (verbatim, current export as of 2026-07): `Action,Time (UTC),ISIN,Ticker,Name,Notes,ID,No. of shares,Price / share,Currency (Price / share),Exchange rate,Total,Currency (Total),Withholding tax,Currency (Withholding tax),Currency conversion fee,Currency (Currency conversion fee)`; row: `Market buy,2026-06-29 13:30:26+00:00,US5949181045,MSFT,"Microsoft",,EOF53409493596,0.0644277800,377.6000000000,USD,1.14054991,21.36,"EUR",,,0.03,"EUR"`. Older/other exports were seen with a plain `Time` column instead of `Time (UTC)` — both are accepted.
- **Quirks**: cleanest export of all — but the column set **varies with the user's export options**: match by header name, never by index; 365 days max per file. **Only trade rows** (`Market/Limit/Stop buy|sell`) are usable for holdings — `Dividend (Dividend)` and `Deposit` rows carry no ISIN/quantity and are skipped. Separately, Trading 212 **Pies** export a *current-snapshot-only* CSV (`Slice,Name,Invested value,Value,Result,Owned quantity,...` + a `Total` summary row) with no ISIN and no dates — useful for eyeballing a Pie's composition, but useless for building history since it carries no transaction dates at all; not supported by the wizard (use the transaction-history export instead, which already reconstructs both current holdings and monthly history).
