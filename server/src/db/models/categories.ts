...
const CATEGORY_SELECT = "id, label, parent_tag:tags!user_categories_parent_tag_id_fkey(client_index)"

function toCategory(row: any) {
    return {id: row.id as number, parentIndex: row.parent_tag?.client_index as number, label: row.label as string}
}
...
export default {getAllByUserId, insertNew, deleteById}
