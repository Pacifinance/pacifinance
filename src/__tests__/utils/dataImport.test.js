...
describe('Auto-Detect Columns', () => {
  describe('detectTableStructure', () => {
    it('falls back safely when no row looks like a transaction table', () => {
      const result = detectTableStructure([['Account report'], ['generated today'], ['notes only']]);
      expect(result).toMatchObject({headerRowIndex: 0, dateCol: null, amountCol: null, dateFormat: null});
    });

    it('finds a header after bank metadata and blank-looking preamble rows', () => {
      const allRows = [
        ['Account holder', 'Mario Rossi', '', ''],
        ['IBAN', 'IT00X0000000000', '', ''],
        ['Report period', '01/01/2025 - 31/03/2025', '', ''],
        ['Booking date', 'Description', 'Debit', 'Credit'],
        ['02/01/2025', 'Coffee shop', '3.50', ''],
        ['05/01/2025', 'Salary', '', '1800.00'],
      ];
      const result = detectTableStructure(allRows);
      expect(result.headerRowIndex).toBe(3);
      expect(result.dateCol).toBe(0);
      expect(result.dualAmountColumns).toEqual({outflowCol: 2, incomeCol: 3});
      expect(result.dateFormat).toBe('DD/MM/YYYY');
    });

    it('detects a table from its values even when every heading is unfamiliar', () => {
      const allRows = [
        ['Export generated by ExampleBank'],
        ['Field A', 'Field B', 'Field C'],
        ['2025-01-02', 'Grocery store', '-42.80'],
        ['2025-01-03', 'Incoming transfer', '900.00'],
        ['2025-01-04', 'Train ticket', '-18.50'],
      ];
      const result = detectTableStructure(allRows);
      expect(result).toMatchObject({headerRowIndex: 1, dateCol: 0, amountCol: 2, dateFormat: 'YYYY-MM-DD'});
    });
  });

  describe('autoDetectColumns', () => {
    it('prefers transaction values over an earlier numeric reference column', () => {
      const headers = ['Reference number', 'When', 'Details', 'Balance change'];
      const rows = [
        ['100234', '2025-01-02', 'Grocery store', '-42.80'],
        ['100235', '2025-01-03', 'Incoming transfer', '900.00'],
      ];
      const result = autoDetectColumns(headers, rows);
      expect(result.dateCol).toBe(1);
      expect(result.amountCol).toBe(3);
    });
