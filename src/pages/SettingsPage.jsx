...
const SettingsPage = () => {
    ...
    const handleGenerateID = async (event) => {
        ...
        try {
            ...
            const response = await axios.post('/user/set-id', data, { withCredentials: true });
            const newIDValue = response.data.new_id;
            setNewID(newIDValue);
            setShowIDResult(true);
            setPassword('');
            setShowChangeID(false);
            setSuccessMessage(languages[language].sidebar.changeID.successPopup.message + newIDValue);
    ...
    const handleChangePassword = async (event) => {
        ...
        try {
            if (password === confirmPassword) {
                ...
                const response = await axios.post('/user/set-password', data, { withCredentials: true });
                if (response.status === 200) {
                    setShowChangePassword(false);
                    setPassword('');
                    setOldPassword('');
                    setConfirmPassword('');
                    setSuccessMessage(languages[language].sidebar.changePassword.successPopup.message);
                    setTimeout(() => setSuccessMessage(''), 5000);
                } else {
            ...
            <StyledSection theme={theme}>
                ...
                <div style={{
                    ...
                    <div style={{ marginBottom: '2rem', padding: '1.5rem', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
                        <h3 style={{ marginBottom: '1rem', color: '#333' }}>Tema e Aspetto</h3>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                            <label style={{ fontWeight: 'bold', color: '#333' }}>
                                {languages[language].sidebar.settings.light}
                            </label>
                            <ToggleModeButton theme={theme} mode={mode} toggleMode={toggleMode} />
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                            <label style={{ fontWeight: 'bold', color: '#333' }}>
                                {languages[language].sidebar.settings.privacy}
                            </label>
                            <PrivacyToggleModeButton theme={theme} mode={mode} toggleHidden={toggleHidden} isHidden={isHidden} />
                        </div>
                    ...
                    <div style={{ marginBottom: '2rem', padding: '1.5rem', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
                        ...
                        <div style={{ marginBottom: '1rem' }}>
                            <MyButton 
                                theme={theme} 
                                onClick={() => setShowChangeID(!showChangeID)}
                                disabled={['test', 'demo'].includes(userType)}
                                style={{ 
                                    width: '100%', 
                                    marginBottom: '0.5rem',
                                    backgroundColor: ['test', 'demo'].includes(userType) ? '#d3d3d3' : theme.buttonBackgroundColor
                                }}
                            >
                    ...
                    {/* Danger Zone */}
                    <div style={{ marginBottom: '2rem', padding: '1.5rem', backgroundColor: '#fff5f5', borderRadius: '8px', border: '1px solid #feb2b2' }}>
                        <h3 style={{ marginBottom: '1rem', color: '#dc3545' }}>Zona Pericolosa</h3>
                        <MyButton 
                            onClick={() => setShowDeleteAccount(!showDeleteAccount)}
                            disabled={['test', 'demo'].includes(userType)}
                            style={{ 
                                backgroundColor: ['test', 'demo'].includes(userType) ? '#d3d3d3' : '#dc3545',
                                color: 'white',
                                width: '100%'
                            }}
                        >
                        ...
                        {showDeleteAccount && (
                            <div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: 'white', borderRadius: '8px' }}>
                                <p style={{ color: '#dc3545', marginBottom: '1rem' }}>
                                    {languages[language].sidebar.deleteAccount.info}
                                </p>
                    ...
                    <div style={{ textAlign: 'center' }}>
                        <MyButton theme={theme} onClick={() => navigate('/dashboard')}>
