...
const DashboardCompactView = ({
  ...
  const allAssetRows = [
    ...traditionalAssets.filter(a => a.value > 0).map(a => ({
      ...a,
      category: translations?.dashboard?.liquidity || 'Liquidity',
    })),
    ...(emergencyFundAsset.value > 0 ? [{
      ...emergencyFundAsset,
      category: translations?.dashboard?.emergencySecurity || 'Emergency',
    }] : []),
    ...investments.map(inv => ({
      ...inv,
      category: translations?.general?.investments || 'Investments',
    })),
  ];
  ...
  return (
    <CompactContainer>
        ...
        <Table>
          <Thead theme={theme}>
            <tr>
              ...
              <Th theme={theme}>{t.category || 'Category'}</Th>
              <Th theme={theme} $align="right">{t.value || translations?.general?.value || 'Value'}</Th>
              <Th theme={theme} $align="right">{t.ofTotal || '% of Total'}</Th>
            </tr>
          </Thead>
          <tbody>
            ...
            <TrTotal theme={theme}>
              <Td theme={theme}>
                <strong>{translations?.general?.total || 'Total'}</strong>
              </Td>
              <Td theme={theme} />
              <Td theme={theme} $align="right">
        ...
        <Table>
          <Thead theme={theme}>
            <tr>
              ...
              <Th theme={theme} $align="right">{t.value || translations?.general?.value || 'Value'}</Th>
              <Th theme={theme} $align="right">{t.ofTotal || '% of Total'}</Th>
              <Th theme={theme} $align="right">{t.vsPrevMonth || 'vs prev. month'}</Th>
            </tr>
          </Thead>
          <tbody>
            {[
              {
                key: 'liquidity',
                label: translations?.dashboard?.liquidity || 'Liquidity',
              ...
              ...(totalEmergencySecurity > 0 ? [{
                key: 'emergency',
                label: translations?.dashboard?.emergencySecurity || 'Emergency & Security',
              ...
              {
                key: 'investments',
                label: translations?.general?.investments || 'Investments',
      ...
      <SectionTitle theme={theme} style={{ marginTop: '1.5rem' }}>
        💶 {translations?.dashboard?.titleGraph3 || 'Income | Outflows'}
      </SectionTitle>
      <TableWrapper theme={theme}>
        <Table>
          <Thead theme={theme}>
            <tr>
              <Th theme={theme}>{t.type || 'Type'}</Th>
              <Th theme={theme} $align="right">{t.amount || 'Amount'}</Th>
              <Th theme={theme} $align="right">{t.ofIncomes || '% of Income'}</Th>
            </tr>
          </Thead>
          <tbody>
            {(() => {
