...
const MonthInput = styled.input`
  min-height: 36px; min-width: 148px; border-radius: 8px; padding: .35rem .55rem;
  border: 1px solid ${(p) => p.theme.mode === 'dark' ? 'rgba(255,255,255,.2)' : 'rgba(15,23,42,.16)'};
  background: ${(p) => p.theme.mode === 'dark' ? '#252b33' : '#fff'};
  color: ${(p) => p.theme.textColor}; color-scheme: ${(p) => p.theme.mode};
  font: inherit; font-weight: 600;
  &::-webkit-calendar-picker-indicator { cursor: pointer; opacity: .85; }
  &:focus-visible { outline: 2px solid ${(p) => p.theme.buttonBackgroundColor}; outline-offset: 2px; }
`;
...
function InOutChart({theme, userData, isHidden}: InOutChartProps) {
  ...
  const renderChart = () => {
    if (view === 'categories') return loadingMonth ? <div aria-live="polite">{t.loading}</div> : <ResponsiveContainer width="100%" height="100%"><BarChart data={categoryRows} layout="vertical" margin={{top: 5, right: 25, left: isMobile ? 2 : 35, bottom: 5}}>
      ...
      <Tooltip trigger={isMobile ? 'click' : 'hover'} formatter={(value: number) => categoryUnit === 'percent' ? `${value.toFixed(1)}%` : money(value)} contentStyle={{background: theme.mode === 'dark' ? '#181e26' : '#fff', color: theme.textColor, borderColor: theme.mode === 'dark' ? 'rgba(255,255,255,.12)' : 'rgba(15,23,42,.1)', borderRadius: 8, fontSize: 12}} itemStyle={{color: theme.textColor}} labelStyle={{color: theme.textColor, fontWeight: 700}} />
      <Bar dataKey="chartValue" name={categoryUnit === 'percent' ? t.percentage : t.value} radius={[0, 5, 5, 0]}>{categoryRows.map((row) => <Cell key={row.key} fill={row.key === '__other__' ? theme.textColor : getCategoryColor(row.key, language)} />)}</Bar>
