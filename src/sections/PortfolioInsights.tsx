...
const Stat = styled.div`
  ...
  .value.positive { color: ${(p) => p.theme.successColor}; }
  .value.negative { color: ${(p) => p.theme.dangerColor}; }
`;
...
const BestWorstRow = styled.div`
  ...
  .positive { color: ${(p) => p.theme.successColor}; }
  .negative { color: ${(p) => p.theme.dangerColor}; }
`;
...
const TargetSummary = styled.div`
  ...
  strong { display: block; margin: 0.2rem 0; color: ${(p) => p.theme.successColor}; font-size: 1.25rem; }
  small { opacity: 0.72; }
  .success-track { height: 7px; margin: 0.75rem 0 0.55rem; border-radius: 999px; overflow: hidden; background: rgba(127,127,127,0.18); }
  .success-fill { height: 100%; border-radius: inherit; background: ${(p) => p.theme.successColor}; transition: width 0.3s ease; }
...
const MonthResult = styled.div<{ $hit: boolean }>`
  ...
  border-left: 3px solid ${(p) => p.$hit ? p.theme.successColor : p.theme.dangerColor};
  font-size: 0.75rem;
  .result-copy { display: flex; flex-direction: column; align-items: flex-end; }
  strong { color: ${(p) => p.$hit ? p.theme.successColor : p.theme.dangerColor}; font-size: 0.78rem; }
