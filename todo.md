...
- [ ] appena il backend è pronto: bisogna sistemare i problemi del floating point e la perdita di centesimi nei valori. Nel db ci saranno tutti i valori come numeri interi (valute moltiplicate * 100) (nel caso fissare due valori dopo la virgola). Il db lo manda come numero intero, io per non perdere centesimi lo uso come number per i calcoli e tutto e lo casto a stringa solo per la visualizzazione mettondoci la virgola dopo i due numeri. (c'è un modo migliore?) (se uno ha 1M di valuta a db avrà 100M e visualizzato nel client avrà 1M.00)
- [ ] sistemare ranking, backend ora manda la % quindi il calcolo già fatto devo prendere il dato e utilizzarlo. Per le spese forse cambia qualcosa
